...
export default function ProfileCard(props) {
  ...
  return (
    <Link className={styles.profileCard} href={`/profile/${props.profile.uid}`} target='_blank'>
      ...
      <h3 className={styles.name}><span className='w-26 truncate text-base' style={{maxWidth: '11.5rem'}}>{props.profile.firstname} {props.profile.lastname}</span><Image className={styles.blueTick} onMouseOver={() => setDisplay('flex')} onMouseOut={() => setDisplay('none')} src='/tickx.png' height='40' width='40' alt="verified-tick" />{props.profile.featured && <span className={styles.container}>
